...
export default function EmployeeDashboardPage() {
    ...
    const fetchEmployeeTickets = async () => {
      ...
      try {
        ...
        if (!querySnapshot.empty) {
          ...
          // ✅ assign employeeId into ticketData
          setTicketData((prev) => new TicketModel({
            ...prev,
            employee_id: employeeInstance.employeeId
          }));
          ...
          <Col md={4} className="p-0">
            <Container className="container custom-container">
              <Container className="body-container">
                {selectedEmployee?.canGenerate === false ? (
                  // 🚫 Notification box if employee is restricted
                  <div className="alert alert-warning mt-4 p-3 rounded-3 shadow-sm">
                    <h5 className="fw-bold mb-2">Access Restricted</h5>
                    <p className="mb-0">
                      You are not authorized to generate QR codes.
                      Please contact your <strong>company administrator</strong> to enable this feature.
                    </p>
                  </div>
                ) : (
                  // ✅ Show form if employee can generate
                  <Form className="custom-form mt-4">
                    <p className="barabara-label">TOURIST ACTIVITY FORM</p>
                    <p className="sub-title-blue">
                      Fill-out this form to generate <strong>QR code</strong>. Accurate information is vital in ensuring safety,
                      maintaining reliable records, generating tour activity reports, providing immediate response in case of
                      emergency, investigating complaints, and optimizing overall service quality.
                    </p>
  ...
  options={[
    {
      ...
      label: `${selectedEmployee.firstname} ${selectedEmployee.surname} — ${
  ...
  isClearable={false}   // removes clear option
  isDisabled={true}     // makes it read-only
/>
                    ...
                    {/* Step 2 */}
                    {currentStep === 2 && (
                      <TicketAddressForm groupData={ticketData} setGroupData={setTicketData} ifForeign={true} />
                    )}

                    {/* Step 3 */}
                    {currentStep === 3 && <TicketActivitiesForm groupData={ticketData} setGroupData={setTicketData} />}
