...
import { Table, Spinner, Button, Dropdown, Card } from "react-bootstrap";
...
export default function CompanyCertSummaryTable({ companies = [], loading = false }) {
  ...
  useEffect(() => {
    const fetchCertData = async () => {
      try {
        const allCertIds = companies.flatMap(c => c.tourism_certificate_ids || []);
        ...
        console.error("Error fetching company certificate data:", error);
      }
    };
  ...
  const handleDownloadImage = async () => {
    ...
    try {
      ...
      download(dataUrl, "company_cert_summary.png", "image/png");
    } catch (err) {
  ...
  const handleDownloadExcel = () => {
    ...
    XLSX.utils.book_append_sheet(workbook, worksheet, "CompanySummary");
    XLSX.writeFile(workbook, "company_cert_summary.xlsx");
  };
  ...
  if (loading) {
    return (
      <div className="text-center my-5">
        ...
        <p>Loading company certificates...</p>
      </div>
    );
  }
    ...
    <>
      ...
      <Card className="summary-card border rounded p-3 text-muted mb-4">
        <div className="d-flex justify-content-between align-items-center mb-2">
          <h6 className="mb-0 text-center w-100">
            Monthly Approved Companies (with issued certificate)
          </h6>
          <Button
            variant="light"
            size="sm"
            onClick={() => handleChartDownload(barChartRef, "monthly_company_cert_bar_chart")}
      ...
      <Card className="summary-card border rounded p-3 text-muted mb-4">
        <div>
          <div className="d-flex justify-content-between align-items-center mb-2">
            <h6 className="mb-0">Company Certificates Summary (with issued certificate)</h6>
            {months.length > 0 && (
              <Dropdown>
