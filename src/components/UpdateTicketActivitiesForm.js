...
const UpdateTicketActivitiesForm = ({ groupData, setGroupData }) => {
    ...
    useEffect(() => {
        const fetchActivitiesAndProviders = async () => {
            ...
            // Step 2: Collect all IDs from existing ticket
            const existingIds = (groupData.activities || [])
                .flatMap(a => a.activities_availed || [])
                .map(act => (typeof act === "string" ? act : act.activity_id))
                .filter(Boolean);

            // Step 3: Fetch missing activities
            const missingIds = existingIds.filter(id => !options.some(opt => opt.value === id));
            if (missingIds.length > 0) {
                ...
                for (const chunk of chunks) {
                    ...
                    const missingSnap = await getDocs(q);
                    const missingOptions = missingSnap.docs.map(doc => {
                        const data = doc.data();
                        return {
                            value: doc.id,
                            label: data.activity_name,
            ...
            // Step 4: Fetch providers for all loaded activities
            const providerIds = [];
            (groupData.activities || []).forEach(a => {
                const act = typeof a.activities_availed?.[0] === "string"
                    ? options.find(opt => opt.value === a.activities_availed[0])?.data
                    : a.activities_availed?.[0];
                if (act?.activity_providers) {
                    providerIds.push(...act.activity_providers);
                }
            });
                    ...
                    return (
                        <div key={index} className="border p-3 mb-4 rounded bg-light">
                            ...
                            <Select
                                ...
                                value={
                                    activityOptions.find((opt) => {
                                        const availed = actGroup.activities_availed?.[0];
                                        const id = typeof availed === "string" ? availed : availed?.activity_id;
                                        return opt.value === id;
                                    }) || null
                                }
    ...
    <Form.Label className="mb-2 text-muted" style={{ fontSize: "0.7rem" }}>
        ...
        for {actGroup.activities_availed[0].activity_duration || ""}
                            ...
                            {(() => {
                                ...
                                // Try to get the activity object from current data
                                const activity =
                                    typeof availed === "object" && availed !== null
                                        ? availed
                                        : activityOptions.find(opt => opt.value === id)?.data;

                                if (!activity) return null;

                                return (
                                    <Form.Label className="mb-2 text-muted" style={{ fontSize: "0.7rem" }}>
                                        SRP Php {Number(activity.activity_price || 0).toLocaleString()} sold by {activity.activity_sold_by}, up to {activity.activity_maxpax} pax
                                        for {activity.activity_duration || ""}
                                    </Form.Label>
                                );
                            })()}
                            ...
                            <Form.Select
                                className="my-2"
                                value={actGroup.activity_area || ""}  // ✅ default to empty string
                                onChange={(e) =>
                                    handleActivityGroupChange(index, { activity_area: e.target.value })
                                }
                                disabled // ✅ if view-only
                            >
                            ...
                            <>
                                ...
                                <Select
                                    ...
                                    isMulti
                                    value={
                                        Array.isArray(actGroup.activity_selected_providers)
...
export default UpdateTicketActivitiesForm;
